...
public class JobCompletionNotificationListener implements JobExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            ...
            List<Object[]> resultList = em.createQuery("select m.team1, count(*) from Match m group by m.team1")
                    .getResultList();
            resultList.stream().map(res -> new Team((String) res[0], (long) res[1]))
            ...
            em.createQuery("select m.team2, count(*) from Match m group by m.team2", Object[].class).getResultList()
                    .stream().forEach(res -> teamMap.compute((String) res[0], (k, team) -> {
                        if (team == null)
